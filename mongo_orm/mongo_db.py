...
import uuid, bson, posts_service.post as posts
import pymongo
...
class MongoRepository(repository.Repository):
    def __init__(self, mongo_connection_string: str, db_name: str, collection_name: str) -> None:
        ...
        self.collection = MongoClient(mongo_connection_string, server_api=ServerApi('1')).get_database(db_name).get_collection(collection_name)
    ...
    def find_all_by_page(self, start_from, page_size: int) -> dict:
        ...
        if start_from == "": 
            ...
            cursor = self.collection.find({"_id": {"$lt": str(start_from)}}).sort(key_or_list=[("date", pymongo.DESCENDING)]).limit(page_size)
